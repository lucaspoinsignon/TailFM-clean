...
Architecture (DiT-style): tokens = timesteps. The input projection Linear(f, d) and the
per-token MLPs mix *channels* at every timestep; multi-head self-attention mixes *time*;
their composition across depth represents joint cross-channel/cross-time interactions --
which is where lagged and contemporaneous co-crash structure must live. Flow time t
conditions every block through adaLN-Zero (per-block shift/scale/gate regressed from a
Fourier embedding of t, gates initialized to zero so the model starts as the identity).
"""
...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = F.scaled_dot_product_attention(q, k, v)          # (B, h, N, dh)
