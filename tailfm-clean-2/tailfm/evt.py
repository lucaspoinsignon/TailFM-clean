...
SHAPE CONSTRAINT.  The GPD MLE is fitted with xi bounded above by `xi_max` (default 0.5,
the finite-variance boundary).  scipy's unconstrained fit is unusable at the sample sizes
available here: with k ~ 60-120 exceedances the MLE for xi has a heavy right tail, and a
true xi of 0.4 routinely returns 2.2 -- or 5.8 -- with no outlier present and the
exceedances decaying perfectly smoothly.  The consequence is not a cosmetic parameter
error.  At xi = 2.2 the fitted survival function decays like y^(-0.46), so an *observed*
exceedance is assigned probability ~1e-12, and T_nu^{-1} of that is |z| ~ 1e4.  Since the
flow is trained on z under a squared-error loss, one such observation contributes ~1e8
times a typical one, dominates the gradient of whichever batch contains it, and destroys
training.  Capping xi is therefore a modelling assumption -- finite variance, which k ~ 100
observations cannot refute anyway -- stated explicitly, rather than a numerical patch.
"""
...
_EPS = 1e-12
XI_MAX = 0.5      # finite variance; raise only with a reason and more exceedances
XI_MIN = -0.4     # below this the fitted upper endpoint sits inside the data
...
def fit_gpd(exc: np.ndarray, xi_max: float = XI_MAX,
            xi_min: float = XI_MIN) -> tuple[float, float]:
    """Profile MLE of (xi, beta) for exceedances over a threshold, xi in [xi_min, xi_max].

    beta is concentrated out through the score equation for the scale, leaving a
    one-dimensional bounded problem in xi -- more robust than scipy's two-parameter
    unconstrained fit, which is what produces the runaway estimates described above.
    Returns (xi, beta); an empty or degenerate exceedance set raises, since silently
    returning a default would hide a marginal that cannot be fitted at all.
    """
    exc = np.asarray(exc, dtype=float)
    exc = exc[np.isfinite(exc) & (exc > 0.0)]
    if exc.size < 2:
        raise ValueError(f"GPD fit needs >=2 positive exceedances, got {exc.size}; "
                         "the threshold coincides with the sample extremum (a point "
                         "mass at the minimum, e.g. an accrual series that never falls)")
    m = exc.mean()

    def nll(xi: float) -> float:
        if abs(xi) < 1e-8:                                   # exponential limit
            beta = m
            return exc.size * np.log(beta) + exc.sum() / beta
        beta = max(m * (1.0 - xi), _EPS)
        # Outside the GPD support the likelihood is zero.  Return a large finite
        # value rather than inf: Brent differences its trial points, and inf - inf
        # is a nan that silently derails the bracket.
        if xi < 0.0 and exc.max() >= -beta / xi:
            return 1e300
        return (exc.size * np.log(beta)
                + (1.0 / xi + 1.0) * np.sum(np.log1p(xi * exc / beta)))

    res = minimize_scalar(nll, bounds=(xi_min, xi_max), method="bounded")
    xi = float(res.x)
    return xi, float(max(m * (1.0 - xi), _EPS))
...
class SemiParametricMarginal:
    ...
    def fit(self, x: np.ndarray) -> "SemiParametricMarginal":
        ...
        # GPD MLE on exceedances, location fixed at 0 (POT), xi capped.
        exc_hi = x[x > self.u_hi_] - self.u_hi_
        exc_lo = self.u_lo_ - x[x < self.u_lo_]
        self.xi_hi_, self.beta_hi_ = fit_gpd(exc_hi, self.xi_max)
        self.xi_lo_, self.beta_lo_ = fit_gpd(exc_lo, self.xi_max)
        self.n_exc_hi_, self.n_exc_lo_ = exc_hi.size, exc_lo.size
        ...
        for j in range(f):
            try:
                self.marginals_.append(
                    SemiParametricMarginal(self.q_tail, self.nu_,
                                           self.xi_max).fit(cols[:, j]))
