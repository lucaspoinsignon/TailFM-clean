...
If F_j were the true CDF then F_j(X_j) ~ U(0,1) exactly, hence
z = T_nu^{-1}(F_j(X_j)) ~ t_nu exactly.  Nothing in the pipeline checks this, yet
it is the assumption the whole z-space construction rests on: the flow is trained
on z and the loss is a squared error, so a mis-specified marginal does not stay
confined to that marginal -- one observation at |z| = 1e4 contributes 1e8 times a
typical one and dominates the gradient for its batch.

Reported per feature, on train and on the held-out rows separately:

    KS       sup_u |F_hat_n(u) - u| on the PIT values, with its p-value.  The
             train p-value is optimistic (the marginal was fitted on those rows)
             and is not a valid test; the TEST column is the one to read.
    AD       Anderson-Darling style weighted statistic, which unlike KS puts most
             of its weight in the tails -- the region that matters here.
    max|z|   the largest transformed value.  Under t_nu with n ~ 1200 rows the
             expected maximum is ~8-15; anything past ~50 means F_j returned a
             probability the data contradicts, i.e. the tail fit is degenerate.
    n_beyond number of test observations outside the training range, where the
             GPD is extrapolating rather than interpolating.

Sorted by test AD, so the worst-fitted features come first.
"""
...
from tailfm.evt import MarginalEnsemble   # direct: avoids the torch
                                          # imports in tailfm/__init__
...
def main():
    ...
    p.add_argument("--no-shrink", action="store_true",
                   help="disable empirical-Bayes pooling of xi across features "
                        "(must match check.py / fit_returns.py)")
    ...
    if a.marginals:
        ...
        # Apply the SAME pooling fit_returns.py applies, otherwise this scores a
        # different marginal than the model is trained on and a --shrink-c sweep
        ...
        print(f"fitted marginals on train rows (nu={marg.nu_:.3f}, "
              f"q_tail={a.q_tail}, "
