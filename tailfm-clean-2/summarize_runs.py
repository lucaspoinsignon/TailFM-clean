"""One table across every run directory: clustering, tail dependence, portfolio risk.

    python summarize_runs.py --data data/returns_clean.csv \
        runs/nu5 runs/nu5_phi08 runs/nu5_phi095 runs/ab_old runs/ab_pos runs/ab_phi runs/ab_both

Everything is scored against the TRAIN windows -- the target distribution for a
scenario generator -- and every column has a stated reference:

  sqACF1/2   mean over features of the within-window squared-return ACF, MINUS the
             iid null -1/(n-1).  Reference: the same quantity on the real training
             windows.  A value of 0 means the generated windows are exchangeable in
             time (what mix_phi=1 produces); the target is the `real` row.

  lam        mean lambda_hat(q0) over all f(f-1)/2 pairs, and corr / slope against
             the real pairwise values.  Reference: the `real` row for lam, and 1.0
             for slope -- but note that a POPULATION-correct generator scores about
             corr 0.91 / slope 0.86 at n=1236 because lambda_hat(real) is measured
             with error; values far above that indicate the sample copula has been
             memorised rather than the population one.

  err/floor  mean|lam_gen - lam_real| divided by mean|lam_halfA - lam_halfB|.  The
             null splits the real rows into two halves of n/2, so a perfect model
             scores ~0.5, NOT 1.0.  Below ~0.5 means the generator reproduces the
             sampling noise of the training copula.

  VaR/CVaR   h-step equal-weight portfolio loss at alpha=0.995, from the generated
             windows.  No reference: the real training sample has only ~50
             non-overlapping h-step blocks, so its own quantile at this level is
             not estimable.  Read it as a comparison ACROSS runs.
"""
...
def main():
    ...
    print(f"\nreferences: sqACF -> the 'real' row (0.0 = time-exchangeable);  "
          f"err/flr -> ~0.5 for a population-correct model, <0.5 = memorisation;\n"
