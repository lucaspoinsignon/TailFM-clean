...
if __name__ == "__main__":
    main()def pseudo_obs(x: np.ndarray) -> np.ndarray:
    n, f = x.shape
    u = np.empty((n, f))
    for j in range(f):
...
def main():
    ...
    nonpos = px.columns[(px <= 0).any(axis=0)]
    if len(nonpos):
        print(f"  drop {len(nonpos):4d}  non-positive price     e.g. {list(nonpos[:4])}")
        px = px.drop(columns=nonpos)
    if px.shape[1] == 0:
        raise SystemExit("no strictly positive price series left")
