...
PAIR SELECTION.  The per-pair figure is the one that does not survive a wide
cross-section: f=200 gives 19900 pairs, and evaluate.tail_dependence_curve
re-ranks the whole (N, f) array once per pair, so tail_dependence_report is
O(N f log N) per pair and never returns.  tail_dependence_figure therefore ranks
pairs by

    d_ij = |lambda_gen(q0) - lambda_real(q0)|      at q0 = screen_q

and keeps the worst `max_pairs`.  Pseudo-observations are computed once per
sample and the screen is a single f x f indicator product, so the cost is
O(N f^2) once instead of per pair; full lambda(q) curves are then evaluated only
for the survivors.  With several generators a pair scores as the worst over
generators, so it is kept if *any* model fails it.  max_pairs=None restores the
old exhaustive behaviour.
"""
